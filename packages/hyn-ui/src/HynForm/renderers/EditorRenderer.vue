<template>
  <component :is="Editor" v-model="modelValue" :min-height="editorField.minHeight || 192" />
</template>

<script setup lang="ts">
import { getHynHostComponent } from '@7mrlan/hyn-ui/runtime';
import type { HynFormEditorField, HynFormModel, HynFormRendererProps } from '../types';
import { getFormFieldValue, setFormFieldValue } from '../utils';

const props = defineProps<HynFormRendererProps>();
const Editor = getHynHostComponent('editor');

// 将通用 HYN 字段收敛为当前渲染器的字段契约，避免模板读取不属于该控件的配置。
const editorField = computed(() => props.field as HynFormEditorField<HynFormModel>);
